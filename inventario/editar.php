...
// Obtenemos el medicamento usando el modelo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Usamos el método POO del modelo
    if ($medModel->actualizar($id, $nombre, $presentacion, $concentracion, $categoria_id, $stock, $stock_min)) {
        ...
        $med = $medModel->obtenerPorId($id); // Recargar datos
    } else {
...
<style>
    body { background-image: linear-gradient(rgba(255, 255, 255, 0.5), rgba(255, 255, 255, 0.5)), url('https://i.imgur.com/ArjbuJ2.jpeg'); background-size: cover; background-position: center; background-attachment: fixed; min-height: 100vh; }
...
<div class="glass-card">
    ...
    <?php if ($mensaje): ?>
        <p style="background: #d4edda; color: #155724; padding: 10px; border-radius: 5px;"><strong><?= $mensaje ?></strong></p>
    <?php endif; ?>
    ...
    <form method="POST">
        ...
        <select name="presentacion" class="form-control" required>
            ...
            foreach ($op as $o) {
                echo "<option value='$o' " . ($med['presentacion'] == $o ? 'selected' : '') . ">$o</option>";
            }
            ?>
        </select>
